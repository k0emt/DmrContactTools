...
function contactWriter(generatedContactList) {
    ...
    // write the header line and strip it from the array
    writeStream.write(
        'Radio ID,Callsign,Name,Nickname,City,State,Country,Remarks<br/>\n'
    )
    generatedContactList.shift()

    // write each value of the array on the file breaking line
    generatedContactList.forEach((contact) =>
        writeStream.write(`${formatContact(contact)}\n`)
    )
...
function formatContact(contact) {
    let noQuotes = contact.replace(/\"/g, '')
    let contactParts = noQuotes.split(',')

    return `${contactParts[1]},${contactParts[2]},${
        contactParts[3].split(' ')[0]
    },,${contactParts[4]},${contactParts[5]},${contactParts[6]},</br>`
}
